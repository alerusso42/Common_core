...
/*   Updated: 2025/02/13 15:55:25 by alerusso         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_bfs(t_map *map, int enemy_num)
{
	...
	while ((++x) || (1))
	{
		...
		if (map->position[x][y].distance[enemy_num] == -1)
			printf("\033[1;33m%2d\033[0m | ", map->position[x][y].distance[enemy_num]);
		else if (map->position[x][y].distance[enemy_num] == -2)
			printf("\033[1;31m%2d\033[0m | ", map->position[x][y].distance[enemy_num]);
		else if (map->position[x][y].distance[enemy_num] == -3)
			printf("\033[1;32m%2d\033[0m | ", map->position[x][y].distance[enemy_num]);
		else
