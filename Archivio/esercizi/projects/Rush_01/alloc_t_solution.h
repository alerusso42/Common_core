...
/*   Updated: 2024/11/04 21:58:20 by alerusso         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	alloc_sol(t_input **input, t_solution **solution)
{
	...
	size = (((*input)->game_size) + 2) * (((*input)->game_size) + 2);
	(*solution) = malloc(sizeof(t_solution));
	if ((*solution) != NULL)
		(*solution)->position = malloc(sizeof(t_solution *) * (size));
	...
	index = -1;
	while (++index != (*input)->game_size + 1)
	{
		(*solution)->position[index] = malloc(sizeof(t_position *) * (size));
