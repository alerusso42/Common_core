...
int	alloc_map(t_input **input, t_map **map)
{
	...
	if (((*map) == NULL) || ((*map)->position) == NULL)
		return (full_reset(2, input, map), l_printf("MALLOC fail map\n"), 3);
	index = -1;
	while (++index != (*input)->game_size_w + 1)
	{
		...
		if ((*map)->position[index] == NULL)
			return (full_reset(2, input, map), l_printf("MALLOC f. map\n"), 3);
	}
...
static int	alloc_copy(t_input **input, t_map **map, int size_w, int size_h)
{
	...
	if (((*map)->old_pos) == NULL)
		return (full_reset(2, input, map), l_printf("MALLOC fail map_c\n"), 3);
	index = -1;
	while (++index != size_w + 1)
	{
		...
		if ((*map)->old_pos[index] == NULL)
			return (full_reset(2, input, map), l_printf("MALLOC f. m.\n"), 3);
	}
	return (0);
}
